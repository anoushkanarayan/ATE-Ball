...
target_angle = 0
line_length = 0.5
line_position = (0, 0)  # Default position in normalized space
    ...
    while cap.isOpened():
        ...
        if lines is not None:
            ...
            with lock:
                ...
                # Normalize position to [-1, 1] range for Matplotlib
                line_position = ((mid_x - frame_center[0]) / (frame_width / 2),
                                 -(mid_y - frame_center[1]) / (frame_height / 2))  # Invert Y for Matplotlib
...
def animate(i):
    ...
    with lock:
        angle = -target_angle  # Flip direction if needed
        length = line_length
        pos_x, pos_y = line_position  # Retrieve position

    # Calculate line coordinates with rotation and translation
